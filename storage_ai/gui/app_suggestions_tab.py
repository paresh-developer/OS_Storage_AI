...
from storage_ai.app_suggestions import AppSuggestion
...
_COLUMNS = ["Application", "Discovered path", "Category / service", "How it was found", "Confidence", "Suggestion"]
...
class AppSuggestionsTab(QWidget):
    ...
    def _show_info(self) -> None:
        QMessageBox.information(self, "About: App Data Suggestions", _INFO_DETAILS)
    ...
    def _on_finished(self, suggestions: list[AppSuggestion]) -> None:
        ...
        self._status_label.setText(
            f"Done -- {len(suggestions)} suggestion(s) found."
            if suggestions
            else "Done -- no actionable suggestions found among the applications currently running."
        )
    ...
    def _populate_table(self) -> None:
        ...
        for row, suggestion in enumerate(self._suggestions):
            ...
            self._set_cell(row, 3, f"{suggestion.source} ({suggestion.detail})")
            self._set_cell(row, 4, f"{suggestion.confidence:.2f}")
            self._set_cell(row, 5, suggestion.advice)
